...
from optim.optim import Adam, AdaBound
from optim.label_smoothing import LabelSmoothing

from train_model.train_multi import TrainerMultiDevice


argparser = ArgumentParser(description='TransformerMT')

group = argparser.add_argument_group(title='Training infomation', description='Print infos')
...
group.add_argument('--optimizer', type=str, help='Type of optimizer', choices=['Adam', 'AdaBound'], default='Adam')
...
group.add_argument('--report_every_steps', type=int, help='Print log after every n batches', default=100)
...
group.add_argument('--update_decay', type=int,
                   help='Backpropagate after every n batches, this will merge generated gradients'
                        'of n batches and update once', default=1)
group.add_argument('--beam_size', type=int, help='Beam size for beam search', default=1)
group.add_argument('--length_penalty', type=float, help='Length penalty alpha when translating', default=1.0)
...
group.add_argument('--gpu_memory_limit', type=float,
                   help='Max memory limit ratio for gpu memory usage (GB), '
                        'which will be used as a threshold to empty gpu cache',
                   default='0.9')
...
group = argparser.add_argument_group(title='Data difficulty', description='Data difficulty settings.')
group.add_argument('--difficulty_json', type=str, help='the path of difficulty json file')
group.add_argument('--fold_name', type=str, help='the name tail of fold')

args = argparser.parse_args()

if __name__ == '__main__':
    print('*' * 80)
    print('Set multiprocessing start method to fork ... ', end='')
    ...
    corpus = Corpus(
        ...
        difficulty_json_file = args.difficulty_json,
        device=corpus_device)

    corpus.train_data_difficulty_stats()
    ...
    if len(device_idx) == 1:
        # WARNING: Only multiple GPU versions are supported
        print('[WARNING] Only multiple GPU versions are supported')
    else:
