...
class SearchNode:
    ...
    def length_normalization(self):
        return ((5 + self.steps) / (5 + 1)) ** self.length_penalty
    ...
    def copy(self):
        ...
        node_clone.length_penalty = self.length_penalty
...
class BeamSearch:
    def __init__(self, beams: int, max_seq_length: int,
    ...
    def nodes_to_list(self):
        return sum(list(list(self.node_pool[i].copy() for _ in range(0, self.beams)) for i in range(0, self.beams)), [])

    def routes(self, probability_scores: torch.Tensor):
        probability_scores = probability_scores.squeeze(dim=1)

        if self.time_steps > 0:
            topk_p, topk_idx = probability_scores.topk(k=self.beams, sorted=True)
            ...
            _, sorted_pool_idx = scores.topk(k=self.beams, sorted=True)

            pool.sort(reverse=True)
            self.node_pool = pool[:self.beams]
        else:
            topk_p, topk_idx = probability_scores[0, :].topk(k=self.beams)
    ...
    def remove_same_nodes(self, pool: [SearchNode]):
        ...
        for i in range(0, len(pool), self.beams):
            if all(node.eos_updated for node in pool[i: i + self.beams]):
                new_pool.append(pool[i])
            else:
                new_pool += pool[i: i + self.beams]
        return new_pool
